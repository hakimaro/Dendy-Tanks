...
void LevelScene::update(pressedKeys keys, float dt) {
    ...
    if (bullet != nullptr) {
        ...
        bullet->move(dt);
        if (bullet->position() != position_tmp) {
            bullet = nullptr;
        }
    }
    /*
    for(auto iterator = bullets.begin(); iterator < bullets.end(); ++iterator) {
        iterator->makeMove(dt);
        vector2f bulletPosition = iterator->position();
        iterator->setPosition(marginPosition(iterator->position(), iterator->size(), iterator->speed(), dt));
        if (bulletPosition != iterator->position() || isHitWall(*iterator)) {
            iterator = bullets.erase(iterator);
        }
    }
    m_man.setBullets(bullets);
    */
}
